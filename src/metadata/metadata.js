  ...
  projects3: [
    {
      ...
      contents:
        "LangGraphオーケストレーションフレームワークを使用して、金融データ分析と文書処理のための高度なマルチエージェントAIシステムを開発。SQLクエリ実行、PDF文書検索、データ可視化、インテリジェントなリクエストルーティングのための専門AIエージェントを統合したスケーラブルなFastAPIバックエンドを構築。金融文書分析のために、多言語（英語/日本語）対応のAgentic RAG（Retrieval-Augmented Generation）機能を実装。\n\n**主な貢献:**\n\n* LangGraphを使用した状態ベースのエージェントワークフローシステムを設計し、AIエージェント間の協調的な対話を実現\n* ウェブアプリケーション統合のためのセッション管理とCORSサポートを備えたRESTful APIエンドポイントを構築\n* Matplotlibを使用した金融データの自動可視化とチャート生成を実装\n* 金融文書を抽出・分析するためのPDF処理パイプラインを開発\n* コード品質ツール（Black, Ruff, isort, Basedpyright）を備えた包括的な開発環境をセットアップ",
    },
  ],
  projects3_1: [
    {
